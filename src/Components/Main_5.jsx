...
class Main_5 extends Component {
    ...
    if (query !== "") {
      ...
      for (var i = 0; i < drawings.length; i++) {
        ...
        for (var j = 0; j < drawings[i].tags.length; j++) {
          if (drawings[i].tags[j].tag.toLowerCase().includes(query.toLowerCase())) {
      ...
      for (var i = 0; i < drawings.length; i++) {
        loop:
        for (var j = 0; j < drawings[i].tags.length; j++) {
          if (drawings[i].tags[j].tag.toLowerCase().includes(sortName.toLowerCase())) {
            temp_drawings.push(drawings[i])
            break loop;
          };
        }
  ...
  render() {
    ...
    const categories = () => {
      const cate = ["All", "Water Color", "Figure", "Oil Painting", "Abstract", "Still-life"];
        ...
        <div className="sort">
          ...
          <div className="cate_div">
            ...
            <label
              ...
              className={item === this.state.sortName || item === this.state.query ? "active" : ""}
            >
              {item}
            </label>
          ))}
