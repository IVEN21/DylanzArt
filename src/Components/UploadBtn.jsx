...
import { getCurrrentUser } from "./BackendService/authService";
function UploadBtn({ data_upload, locked, loading: { load, text } }) {
  const item = [
    { char: "U", id: 0 },
    { char: "P", id: 1 },
    { char: "L", id: 2 },
  ...
  const uploadValidation = () => {
    if ( getCurrrentUser() && getCurrrentUser().admin &&locked) {
      data_upload();
    } else if (locked) {
  ...
  if ( load) {
    return (
      <div className="up_grid loding">
        <div>
          <ClimbingBoxLoader color="#73ffc2" size="15px" />
        </div>
        <p>{text}</p>
      </div>
    );
